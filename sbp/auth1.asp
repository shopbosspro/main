...
//alert(isMobile(any))
if (isMobile.any()){
	//alert("auth1mobile2.asp?rs=y&shopid=<%=request("shopid")%>&roid=<%=request("roid")%>")
	location.href = "auth1mobile2.asp?rs=y&shopid=<%=request("shopid")%>&roid=<%=request("roid")%>"
	//location.href='auth1mobile.asp?shopid=<%=shopid%>&roid=<%=roid%>'
}
...
function getRO(){
    ...
    xmlhttp.onreadystatechange = function() {
        if (xmlhttp.readyState == 4 && xmlhttp.status == 200) {
        	...
        	//console.log(invname)
        	setTimeout(function(){
            	document.getElementById("invoice").src = "https://<%=request.servervariables("SERVER_NAME")%>/sbp/pdfinvoices"+invname
...
function signRO(){
	...
	if(agreecheck){
		document.getElementById("spinner").style.display = "block"
		data="roid="+roid+"&shopid="+shopid+"&ph="+last4phone+"&sig="+sig+"&a="+a+"&firstro="+document.getElementById("invoice").src
		console.log(data)
	    var xmlhttp = new XMLHttpRequest();
	    xmlhttp.onreadystatechange = function() {
	        if (xmlhttp.readyState == 4 && xmlhttp.status == 200) {
	        	...
	        	if (results == "validationerror"){
	        		...
	        		document.getElementById("spinner").style.display = "none"
	        		document.getElementById("invresults").value = results
	        		window.scroll(0,0)
...
if (isMobile.any()){
	location.href='auth1mobile2.asp?shopid=<%=shopid%>&roid=<%=roid%>'
}*/
...
<div class="auto-style4">
	...
	<table style="width: 100%" cellpadding="3" cellspacing="0">
		<tr>
			...
			Signature by
	<img alt="" style="max-height:28px" src="http://shopboss.net/wp-content/uploads/2016/04/shop-boss-logo-final-colors-01-copy.png"  align="top" /></span></td>
		</tr>
	</table>
	
	</div>
<div id="header" style="text-align:center" class="auto-style1"><br/><strong><%=shopname%> is requesting an E-Signature on the <%=etitle%> shown below.<br/>To 
add your electronic signature, please complete the information to the left of 
the document.</strong></div>
...
%><br/><iframe id="invoice" style="width:70%;height:560px;float:right;border-left:0px;" src=""></iframe>
...
do until trs.eof
%>
<input value="<%=ucase(trs("val"))%>" id="address<%=c%>" onclick="setNormal('selectaddr')" name="address" type="radio" /> &nbsp;<%=ucase(trs("val"))%><br />
<%
	c = c + 1
	trs.movenext
loop
...
<br />
	...
	<input onfocus="setNormal('last4')"  name="ph" id="ph" type="text" /><br />
	<br />
	<span id="siglabel">Type your name in the box below as your signature</span><br />
	<input onfocus="setNormal('siglabel')" name="sig" id="sig" style="font-size:18px;width: 235px;font-family:'Trebuchet MS’, Helvetica, sans-serif'" type="text" /><br />
	<br />
	<input onclick="setNormal('sigcheck')" id="agreecheck" name="Checkbox1" type="checkbox" /> <span id="sigcheck">Signature 
	...
	<br />
&nbsp;<input name="Button1" type="button" onclick="signRO()" value="I Agree and Authorize" />
	<input name="Button2" type="button" value="Cancel - I DO NOT Authorize" />
